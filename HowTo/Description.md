...
## CHARACTER INSTANCES:
The character instances tab will be visible through the “edit Description” function after a Description node has been submitted. Open the accordion(s) to enter Character States from your chosen Schema. There may be a hierarchical set of accordions depending on how the Schema was designed. If you are scoring character state for a published description, please see the the [guide to entering published data](PublishedData.md) for guidance on translating authors’ text descriptions to standardized schemas.
