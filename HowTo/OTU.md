...
* For formally named, published OTUs, the name should simply be the full published name (i.e., the binomial consisting of genus + specific epithet).
**<u>Importantly, only use the Linnaean binomial name if you are entering the official taxonomic record (whether original or revised), complete with the published reference, diagnosis, and at least one type specimen.<u>** If you are creating an OTU record because you are entering material that is identified as a known taxon that is not yet in the database, then we encourage you to find the original description and type/exemplar specimen and create the public official OTU record for that species. Then you may simply assign the additional new specimens to that taxon. If you are not able to enter the original record for a published/named OTU, as an interim solution you can add a descriptor for your locality to the end of the OTU name (e.g., Platanites raynoldsii Big Bluff) to differentiate it from the official OTU record. You may edit/add to this record if you are later able to find and enter the actual diagnosis, reference, locality and type specimen for the official OTU, at which point you can edit the OTU name to essentially convert it to the official record for the taxon. Alternately, if you discover that someone else has entered the official OTU, then you can reassign your specimens to that OTU node; any Descriptions you have made will carry over, as they are tied to Specimens, not OTUs. Remember to delete your old OTU after reassigning the specimens. Note that if you name an OTU with only the formal genus + species epithet in your private workbench, this prevents anyone else from entering the formally named OTU!
