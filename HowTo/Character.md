...
**Parent Character** – Select the higher-ranked characteropsis the new characteropsis belongs to. Skip this step if you are entering a top-rank organizational header (and thus not a sub-header). Also skip this step for cases where a character is not dependent on any rank of organizational header. Note that as you add more characters to a schema, the drop-down menu for parent character will reflect the hierarchical organization that has been generated by use of bold font for top-level characters and then indented sub-headings and characters.

**Name** – Enter any rank of organizational header name using title case. Enter character name using sentence case. These names should be precise and informative, but concise, as they will appear in the selection drop-down menus when applying a schema to score specimens.
