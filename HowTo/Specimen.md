...
**Notable features preserved** – Choose from the list any notable aspects of the specimen that are known to be preserved. This information can help workers know whether a specimen has/preserves certain key features that can be studied, even if not described yet.
...
**Identified by** – Choose names of people (Person nodes) that are responsible for identifying the specimen as a particular OTU. Note that Specimens are assigned to OTUs through the OTU page.
